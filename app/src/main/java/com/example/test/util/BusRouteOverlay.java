...
public class BusRouteOverlay extends RouteOverlay {
			...
			for (int i = 0; i < busSteps.size(); i++) {
				...
				if (i < busSteps.size() - 1) {
					...
					if (busStep.getWalk() != null
							&& busStep.getBusLines().get(0) != null) {
					...
					if (busStep.getBusLines().get(0) != null
							&& busStep1.getWalk() == null
							&& busStep1.getBusLines().get(0) != null) {
				...
				if (busStep.getWalk() != null
					...
					if (busStep.getBusLines().get(0) == null && busStep.getRailway() == null && busStep.getTaxi() == null) {
	...
	private LatLonPoint getLastBuslinePoint(BusStep busStep) {
		List<LatLonPoint> lonPoints = busStep.getBusLines().get(0).getPolyline();
